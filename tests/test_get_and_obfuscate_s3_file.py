import pytest
from src.get_and_obfuscate_s3_file import get_and_obfuscate_s3_file, InvalidInputError, UnsupportedFileTypeError
...
def test_get_and_obfuscate_s3_file_returns_exception_when_file_format_not_supported():
    ...
    with pytest.raises(UnsupportedFileTypeError) as excinfo:  
        get_and_obfuscate_s3_file(s3_details)  
    assert str(excinfo.value) == "Only csv, json and parquet files supported for obfuscation"