...
def test_create_s3_object_from_dataframe_raises_exception_for_unsupported_file_type(
    sample_df,
):
...
def test_create_s3_object_from_dataframe_returns_data_same_as_passed_df_csv(sample_df):
    response = create_s3_object_from_dataframe(sample_df, "csv")
    new_df = pd.read_csv(io.StringIO(response), index_col=[0])
    assert new_df.equals(sample_df)


def test_create_s3_object_from_dataframe_returns_data_same_as_passed_df_parquet(
    sample_df,
):
    response = create_s3_object_from_dataframe(sample_df, "parquet")
    new_df = pd.read_parquet(io.BytesIO(response))
    assert new_df.equals(sample_df)
